...
secondaryAuth.setPersistence(browserLocalPersistence); // <-- Agregar esto

// Constante para el rol de administrador
const ADMIN_ROLE = import.meta.env.VITE_ADMIN_ROLE || "DhHkVja"; // Valor por defecto por compatibilidad
...
async function isUserAdmin(userId) {
  try {
    ...
    if (userDoc.exists()) {
      ...
      return userData.role === ADMIN_ROLE || userData.role === "admin"; // Doble validación por compatibilidad
    }
    return false;
  } catch (error) {
...
export { db, auth, secondaryAuth, ADMIN_ROLE, isUserAdmin, signIn, loginWithGoogle, firebaseSignOut };